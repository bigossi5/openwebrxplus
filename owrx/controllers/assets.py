...
class AssetsController(GzipMixin, ModificationAwareController, metaclass=ABCMeta):
    ...
    def serve_file(self, file, content_type=None, max_age=3600):
        try:
...
class OwrxAssetsController(AssetsController):
    ...
    def indexAction(self):
        filename = self.request.matches.group(1)
        # plugin init.js is regenerated by the Plugin Manager whenever a
        # plugin is enabled/disabled/installed/uninstalled; keep its cache
        # lifetime short so a plain page refresh picks up the change instead
        # of requiring a full server restart to bust the browser cache
        max_age = 5 if filename.endswith("plugins/receiver/init.js") else 3600
        self.serve_file(filename, max_age=max_age)
...
class CompiledAssetsController(GzipMixin, ModificationAwareController):
    ...
    def indexAction(self):
        ...
        # short cache lifetime: these bundles change whenever the app is
        # redeployed, and a stale copy silently hides new/changed JS behind
        # a full hour of browser caching (a conditional request still avoids
        # re-downloading unchanged content, so this is cheap either way)
        self.send_response("\n".join(contents), content_type=content_type, last_modified=modified, max_age=60)
